...
**Arguments:** [system-name | --summary | (no arg = full audit)]

**Primary role:** Read `../../agents/producer.md` before following this workflow.
...
2. **L0 pre-scan**: Before full-reading any GDDs, Grep all GDD files for `## Summary` sections plus common content-count
   keywords:
   ...
   For a single-system audit: skip this step and go straight to full-read. For a full audit: full-read only the GDDs
   that matched content-count keywords. GDDs with no content-count language (pure mechanics GDDs) are noted as "No
   auditable content counts" without a full read.
...
5. **Build a content inventory table** from the extracted data:
   ...
   Note: If a GDD describes content qualitatively but gives no count, record "Unspecified" and flag it — unspecified
   counts are a design gap worth noting.

---

## Phase 2 — Implementation Scan

For each content type found in Phase 1, scan the relevant directories to count what has been implemented. Use Glob and
Grep to locate files.

**Levels / Areas / Maps:**

- Glob `assets/**/*.tscn`, `assets/**/*.unity`, `assets/**/*.umap`, `assets/**/*.bsn`, `assets/**/*.scn.ron` (Bevy
  scenes)
...
- Counts are approximations — the skill cannot perfectly parse every engine format or distinguish editor-only files from
  shipped content
