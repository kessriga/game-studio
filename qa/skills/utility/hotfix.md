...
None. Hotfixes are time-critical. Director review may follow separately as a post-hoc step. No gate is invoked within
this skill.
...
- The case where multiple files need to be modified for one fix follows the same "May I write" per-file pattern and is
  not separately tested.
- The post-hotfix steps (create bug report, update changelog) are suggested in the handoff but not tested as part of
  this skill's execution.
- Conflict resolution during the merge (if main has diverged) is not tested; the skill would surface the conflict and
  ask the user to resolve it manually.
