...
- [ ] Contains "May I write" or a user-input tool or chat language before writing the adoption plan
...
- `docs/architecture/adr-0001.md` exists with `## Status`, `## Engine Compatibility`, and all other required sections
...
8. Uses a user-input tool or chat to ask about writing the plan; user selects write
...
- [ ] a user-input tool or chat is used before writing the adoption plan
...
- [ ] a user-input tool or chat is used before writing
...
2. Audit summary shows totals: "4 GDDs (2 fully compliant, 2 with gaps); 3 ADRs (2 fully compliant, 1 with gaps); 5
   stories (3 with TR-IDs, 2 without)"
...
5. Note included: "Existing stories continue to work — do not regenerate stories that are in progress or done"
6. a user-input tool or chat to write plan; writes after approval
...
- [ ] Uses a user-input tool or chat before writing the adoption plan file
...
- The `gdds`, `adrs`, `stories`, and `infra` argument modes narrow the audit scope; each follows the same pattern as the
  full audit but limited to that artifact type. Not separately fixture-tested here.
- The systems-index.md parenthetical status value check (BLOCKING) is a special case that triggers an immediate fix
  offer before writing the plan; not separately tested.
- The review-mode.txt prompt (Phase 6b) runs after plan writing if `production/review-mode.txt` does not exist; not
