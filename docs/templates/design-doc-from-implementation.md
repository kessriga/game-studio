...
**Status**: Reverse-Documented **Source**: `[path to implementation code]` **Date**: [YYYY-MM-DD] **Verified By**: [User
name or "pending review"]
**Implementation Status**: [Fully implemented | Partially implemented | Needs extension]
---

> **⚠️ Reverse-Documentation Notice**
>
> This design document was created **after** the implementation already existed. It captures current behavior and
> clarified design intent based on code analysis and user consultation. Some sections may be incomplete where
> implementation is partial or design intent was unclear during reverse-engineering.
...
- Run `/skill:gamedev-balance-check` on [specific aspect]
...
- [ ] **Balance pass**: Run `/skill:gamedev-balance-check` on progression curve
...
| Date | Author | Changes |
|------|--------|---------|
| [Date] | [Author] (reverse-documentation) | Initial reverse-documentation from `[source path]` |
...
- `/skill:gamedev-balance-check` — Validate formulas and progression
- `/skill:gamedev-architecture-decision` — Document technical decisions
- `/skill:gamedev-code-review` — Ensure code matches clarified design

---

*This document was generated by `/skill:gamedev-reverse-document design [path]`*
