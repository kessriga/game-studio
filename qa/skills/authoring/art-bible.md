...
In `full` review mode, the AD-ART-BIBLE director gate (art director) runs after the draft is complete and before any
section is written. In `lean` and `solo` modes, AD-ART-BIBLE is skipped and only user approval is required. The verdict
...
4. Skill drafts updated content; in full mode, AD-ART-BIBLE is invoked for the revised section before writing
...
- The case where AD-ART-BIBLE returns REJECT (not just CONCERNS) is not separately tested; the skill would block writing
