...
Domain: C# patterns in Godot 4, .NET idioms applied to Godot, [Export] attribute usage, signal delegates, and
async/await patterns. Does NOT own: GDScript code (gdscript-specialist), GDExtension C/C++ bindings
(gdextension-specialist). No gate IDs assigned.
...
- Redirects the request to `gamedev:godot-gdscript-specialist`
...
- Notes that the calling method must be `async` and that fire-and-forget `async void` is only acceptable for event
  handlers
...
- Provides the correct pattern: use `CallDeferred()`, `Callable.From().CallDeferred()`, or marshal back to the main
...
- Produces C# signal connection using the typed delegate pattern introduced in Godot 4 C# (`+=` operator on typed
  signal)
