...
The framework installs as the `gamedev` Claude Code plugin: the repo serves as its own marketplace, ships its components from canonical plugin locations, exposes skills and agents under the `gamedev:` namespace, and resolves its framework docs from the install location.
