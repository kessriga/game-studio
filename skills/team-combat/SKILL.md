...
> "Usage: `/skill:gamedev-team-combat [combat feature description]` — Provide a description of the combat feature to
...
- **engine specialist** (primary) — Validate architecture and implementation patterns are idiomatic for the engine (read
  from `docs/technical-preferences.md` Engine Specialists section)
...
- `gamedev:game-designer` — Design the mechanic, define formulas and edge cases
- `gamedev:gameplay-programmer` — Implement the core gameplay code
- `gamedev:ai-programmer` — Implement NPC/enemy AI behavior
- `gamedev:technical-artist` — Create VFX, shader effects, visual feedback
- `gamedev:sound-designer` — Define audio events, impact sounds, ambient audio
- `role: [primary engine specialist]` — Engine idiom validation for architecture and implementation
- `gamedev:qa-tester` — Write test cases and validate implementation
...
- Is the class/node/component structure idiomatic for the pinned engine? (e.g., Godot node hierarchy, Unity
  MonoBehaviour vs DOTS, Unreal Actor/Component design, Bevy ECS components + systems)
...
All file writes (design documents, implementation files, test cases) are delegated to sub-agents spawned through
...
Verdict: **COMPLETE** — combat feature designed, implemented, and validated. Verdict: **BLOCKED** — one or more phases
could not complete; partial report produced with unresolved items listed.

## Next Steps

- Run `/skill:gamedev-code-review` on the implemented combat code before closing stories.
- Run `/skill:gamedev-balance-check` to validate combat formulas and tuning values.
- Run `/skill:gamedev-team-polish` if VFX, audio, or performance polish is needed.
