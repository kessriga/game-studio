...
5. **Routing table content for Bevy**: game code (`*.rs`) → `bevy-rust-specialist` (the Language/Code Specialist row, matching how Godot routes `*.gd` → its GDScript specialist rather than to the primary); shader files (`*.wgsl`) → `bevy-render-specialist`; UI code → `bevy-ui-specialist`; scene/asset data (`*.ron`, `assets/`) → `bevy-specialist` (primary); native-extension row → "N/A (Bevy is native Rust)". The schema is untouched; inapplicable rows state N/A explicitly instead of dangling.
...
8. **Testing-framework specs** are added under the skill/agent testing directory (`qa/`) engine section with `catalog.yaml` entries.
