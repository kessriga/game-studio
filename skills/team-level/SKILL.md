...
**Arguments:** [level name or area to design] [--review full|lean|solo]
...
1. **Read the argument** for the target level or area (e.g., `tutorial`, `forest dungeon`, `hub town`,
   `final boss arena`).
...
- `gamedev:narrative-director` — Narrative purpose, characters, emotional arc
- `gamedev:world-builder` — Lore context, environmental storytelling, world rules
- `gamedev:level-designer` — Spatial layout, pacing, encounters, navigation
- `gamedev:systems-designer` — Enemy compositions, loot tables, difficulty balance
- `gamedev:art-director` — Visual theme, color palette, lighting, asset requirements
- `gamedev:accessibility-specialist` — Navigation clarity, colorblind safety, cognitive load
- `gamedev:qa-tester` — Test cases, boundary testing, playtest checklist
...
Spawn all three agents simultaneously — issue all three delegation calls before waiting for any result.

Spawn the `gamedev:narrative-director` agent to:
...
Spawn the `gamedev:world-builder` agent to:
...
Spawn the `gamedev:art-director` agent to:
...
**Gate**: Use a user-input tool or chat to present all three Step 1 outputs (narrative brief, lore foundation, visual
direction targets) and confirm before proceeding to Step 2.

### Step 2: Layout and Encounter Design (level-designer)

Spawn the `gamedev:level-designer` agent with the full Step 1 output as context:
...
Use a user-input tool or chat with options:
...
**Gate**: Use a user-input tool or chat to present Step 2 layout (including any unresolved adjacent area dependencies)
and confirm before proceeding to Step 3.

### Step 3: Systems Integration (systems-designer)

Spawn the `gamedev:systems-designer` agent to:
...
**Gate**: Use a user-input tool or chat to present Step 3 outputs and confirm before proceeding to Step 4.
...
Spawn the `gamedev:art-director` agent with the finalized layout from Step 2:
...
**Gate**: Use a user-input tool or chat to present both Step 4 results. If the accessibility-specialist returned any
BLOCKING concerns, highlight them prominently and offer:
...
Spawn the `gamedev:qa-tester` agent to:
...
After all subagent outputs are collected, spawn `gamedev:level-designer` through authorized delegation to compile and
...
2. **Output a summary** with: area overview, encounter count, estimated asset list, narrative beats, any cross-team
   dependencies or open questions, open cross-level dependencies (adjacent areas referenced but not yet designed, each
   marked UNRESOLVED), and accessibility concerns with their resolution status.

## File Write Protocol

All file writes (level design docs, narrative docs, test checklists) are delegated to sub-agents spawned through
authorized delegation. Each sub-agent enforces the "May I write to [path]?" protocol. This orchestrator does not write
files directly.

Verdict: **COMPLETE** — level design document produced and all team outputs compiled. Verdict: **BLOCKED** — one or more
agents blocked; partial report produced with unresolved items listed.
