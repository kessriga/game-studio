...
Domain: Menu screens, HUDs, inventory screens, dialogue boxes, UI framework code, and data binding. Does NOT own: UX
flow design (ux-designer), visual style direction (art-director / technical-artist). No gate IDs assigned.
...
- Explicitly states that UX flow design belongs to `gamedev:ux-designer`
...
- Coordinates with `gamedev:technical-artist` for an animation spec (duration, easing curve, overshoot amount)
...
**Input:** The UX spec states "show item details on selection" but does not define what happens when an empty slot is
...
- Flags the ambiguity back to `gamedev:ux-designer` with the specific question: "What should the detail panel show when
  an empty inventory slot is selected?"
...
**Input:** Engine context provided: project uses Godot 4.6 with Control node UI. Request: "Implement a scrollable item
list for the inventory." **Expected behavior:**
