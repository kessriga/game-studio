# Example Session: Full Story Lifecycle — /skill:gamedev-story-readiness → Implement → /skill:gamedev-story-done

**Context:** Solo indie dev, mid-production. The movement system GDD and architecture are complete. A story for
movement's core implementation has been generated by `/skill:gamedev-create-stories`, which also minted its Backlog
task.
...
- `production/epics/movement/story-001-movement.md` — story spec (STORY-MOV-001), carries `Tracked in: TASK-MOV-001`;
  not yet validated
- Backlog task TASK-MOV-001 — status `To Do`, carries `Spec: production/epics/movement/story-001-movement.md` (this task
  owns work-item status)
- No implementation code yet

**Session Duration:** ~50 minutes **Turns:** 13 **Outcome:** Story validated ready, implemented, verified complete with
...
**User:**
> /skill:gamedev-story-readiness production/epics/movement/story-001-movement.md

### Turn 2: Agent Validates the Story

**Agent (via /skill:gamedev-story-readiness):**
...
> - Ambiguities found: **1**
>   - ⚠️ Story says "roll direction follows last input direction" but GDD says "roll in movement direction" — these
>     conflict when player stops and immediately rolls. Clarify before pickup.
>
...
> **Verdict: NEEDS WORK**
>
> One ambiguity must be resolved before implementation starts. Would you like to resolve it now so I can clear
> TASK-MOV-001 for pickup?
...
>
> May I write this to `production/epics/movement/story-001-movement.md`? TASK-MOV-001 stays `To Do` — a task existing
> means it is ready, so readiness is a validation gate, not a status change.
...
> **Verdict: READY** Story is implementation-ready. Pull it off the board when you're ready to build.
...
> Reading `production/epics/movement/story-001-movement.md`, `design/gdd/movement-system.md`, and
...
### Turn 10: User Tests Manually and Runs /skill:gamedev-story-done

**User:**
> I've tested it in the scene — everything works. /skill:gamedev-story-done
> production/epics/movement/story-001-movement.md

### Turn 11: Agent Runs Completion Review

**Agent (via /skill:gamedev-story-done):**
...
>
> Suggest running `/skill:gamedev-code-review src/gameplay/movement/` before marking complete. Not blocking.
>
> ---
...
- Story `.md` — `## Completion Notes` appended (the spec is frozen to status-only; this is the one thing it keeps
  receiving):
...
>
> Run `/skill:gamedev-story-readiness` on whichever you pick up next.
...
- **Readiness gate catches real issues**: the roll direction ambiguity would have caused a late implementation decision
  without `/skill:gamedev-story-readiness`
...
- **Deferred criteria**: not every acceptance criterion can be verified at story close — `/skill:gamedev-story-done`
  tracks deferred items rather than blocking completion
...
- **Backlog board is the status source**: TASK-MOV-001 owns work-item status (`To Do` → `In Progress` at pickup → `Done`
  at close). The story `.md` freezes to a status-only spec that receives `## Completion Notes`; the two stay linked by
  `Spec:` / `Tracked in:` cross-refs. No YAML tracker to keep in sync
