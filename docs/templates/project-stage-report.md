...
- `/gamedev:reverse-document [type] [path]` — [For which gap]
- `/gamedev:architecture-decision` — [For which gap]
- `/gamedev:create-epics` → `/gamedev:create-stories` — [If the Backlog board has no work items yet]
...
*Generated by `/gamedev:project-stage-detect` skill*
