...
Domain: bevy_ui — Node/flexbox layout, widgets (bevy_ui_widgets / bevy_feathers), Parley text & typography, text input
(EditableText), input focus, accessibility (AccessKit). Does NOT own: host-side gameplay Rust (bevy-rust-specialist),
shader/material work (bevy-render-specialist), high-level architecture (bevy-specialist). No gate IDs assigned.
...
**Input:** "Build a HUD bar with a label and a value, centered." **Expected behavior:**
...
**Input:** "Set TextFont { font: handle, font_size: 24.0 }." **Expected behavior:**

- Flags the pre-Parley shape; at 0.18/0.19 it is `TextFont { font: FontSource, font_size: FontSize }` with
  `handle.into()` and `FontSize::Px(24.0)`
- Notes `LineHeight` is now a required component
- Points to `docs/engine-reference/bevy/modules/ui.md`

### Case 3: Text input

**Input:** "Add a text field for the player to type their name." **Expected behavior:**
...
**Input:** "Ship this button." **Expected behavior:**
...
**Input:** "How do we verify the menu looks right?" **Expected behavior:**

- Treats layout/appearance as UI/Visual evidence (manual walkthrough or screenshot), keeps UI *logic* headlessly
  testable
