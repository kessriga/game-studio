...
**Arguments:** [version number or 'next'] [--review full|lean|solo]
...
2. If a version is found: report "No version argument provided — inferred [version] from milestone data. Proceeding."
   Then confirm with a user-input tool or chat: "Releasing [version]. Is this correct?"
3. If no version is discoverable: use a user-input tool or chat to ask "What version number should be released? (e.g.,
   v1.0.0)" and wait for user input before proceeding. Do NOT default to a hardcoded version string.
...
- `gamedev:release-manager` — Release branch, versioning, changelog, deployment
- `gamedev:qa-lead` — Test sign-off, regression suite, release quality gate
- `gamedev:devops-engineer` — Build pipeline, artifacts, deployment automation
- `gamedev:security-engineer` — Security audit for online/multiplayer/data features
- `gamedev:analytics-engineer` — Telemetry event verification and dashboard readiness
- `gamedev:community-manager` — Patch notes and launch communication
- `gamedev:producer` — Go/no-go decision, stakeholder communication
- `gamedev:network-programmer` — Netcode stability sign-off (invoke if game has multiplayer)
...
- Embed the justification text in the partial approval record before Phase 6: append a "⚠️ Override Justification:
  [user's text]" field.
...
All file writes (release checklists, changelogs, patch notes, deployment scripts) are delegated to sub-agents and
sub-skills. Each enforces the "May I write to [path]?" protocol. This orchestrator does not write files directly.
...
Verdict: **COMPLETE** — release executed and deployed. Verdict: **BLOCKED** — release halted; go/no-go was NO or a hard
