...
2. Skill finds GDDs (Systems Design complete), epics (Pre-Production complete), source code and sprints (Production
   active)
...
- The Technical Setup stage (engine configured, no GDDs yet) and Pre-Production stage (GDDs complete, no epics yet)
  follow the same artifact-inference pattern as Cases 2 and 3 and are not separately fixture-tested.
- The Polish and Release stages are not fixture-tested here; they follow the same high-confidence (stage.txt present) or
  inference logic.
