...
**Arguments:** [description] | analyze [path-to-file] | verify [TASK-ID] | close [TASK-ID]

# Bug Report

Bugs are tracked as **Backlog tasks** with the `bug` label — the board is the bug list (there is no separate
`production/qa/bugs/` markdown store). Triage is the board filtered by the `bug` label and sorted by priority. Rich
repro/severity/ context live in the task description and acceptance criteria.
...
3. **For each potential bug**, draft a report (Phase 2A template) with the likely trigger scenario and recommended fix,
   and file it via Phase 3 (one `bug` task each).
...
If STILL PRESENT: `task_edit` the task back to `In Progress` (or `To Do`), append a comment with the evidence, and
suggest `/skill:gamedev-hotfix [TASK-ID]`. If VERIFIED FIXED: proceed to Close Mode (or tell the user to run `close`).
...
`/skill:gamedev-bug-report verify [TASK-ID]` first."

Ask: "May I set [TASK-ID] to Done with a closure note?" If yes, `task_edit`:

- `status`: `Done`
- append a `commentsAppend` closure record: resolution one-liner, fix commit/PR if known, regression test path (or
  "manual verification"), closed-by
...
- If S1 or S2: run `/skill:gamedev-hotfix [TASK-ID]` for the emergency fix workflow

**After fixing the bug (developer confirms fix is in):**

- Run `/skill:gamedev-bug-report verify [TASK-ID]` — confirm the fix actually works before closing
- Never mark a bug Done without verification — a fix that doesn't verify is still open

**After verify returns VERIFIED FIXED:**

- Run `/skill:gamedev-bug-report close [TASK-ID]` — set the task Done and write the closure note
