# Skill Test Spec: /gamedev:team-ui
...
sub-agents and sub-skills (`/gamedev:ux-design`, `ui-programmer`). Produces a summary report
with verdict COMPLETE / BLOCKED and handoffs to `/gamedev:ux-review`, `/gamedev:code-review`,
`/gamedev:team-polish`.
...
- [ ] Has a next-step handoff at the end (references `/gamedev:ux-review`, `/gamedev:code-review`, `/gamedev:team-polish`)
...
**Input:** `/gamedev:team-ui inventory screen`
...
2. Phase 1b — `/gamedev:ux-design inventory-screen` invoked (or ux-designer spawned directly); produces `design/ux/inventory-screen.md` using `ux-spec.md` template; `AskUserQuestion` confirms spec before review
3. Phase 1c — `/gamedev:ux-review design/ux/inventory-screen.md` invoked; returns APPROVED; gate passed, proceed to Phase 2
...
- [ ] Next steps include `/gamedev:ux-review`, `/gamedev:code-review`, `/gamedev:team-polish`
...
- `/gamedev:ux-review` returns verdict NEEDS REVISION with specific concerns flagged (e.g., gamepad navigation flow incomplete, contrast ratio below minimum)

**Input:** `/gamedev:team-ui inventory screen`

**Expected behavior:**
1. Phase 1a + 1b complete — UX spec produced
2. Phase 1c — `/gamedev:ux-review design/ux/inventory-screen.md` returns NEEDS REVISION
...
5. If user chooses (a): ux-designer revises spec, `/gamedev:ux-review` re-run; loop continues until APPROVED or user overrides
...
**Input:** `/gamedev:team-ui inventory screen` (resuming from Phase 3 complete)
...
**Input:** `/gamedev:team-ui settings menu`
...
- [ ] `AskUserQuestion` offers a "create pattern library first" option (referencing `/gamedev:ux-design patterns`)
...
- [ ] Next steps present at end: `/gamedev:ux-review`, `/gamedev:code-review`, `/gamedev:team-polish`

---

## Coverage Notes

- The HUD-specific path (`/gamedev:ux-design hud` + `hud-design.md` template + visual budget check in Phase 5)
