...
All shader files in `assets/shaders/` must follow these standards to maintain visual quality, performance, and
...
- File naming: `[type]_[category]_[name].[ext]`
  ...
  - `env_water.wgsl` (Bevy — snake_case WGSL, paired with a `Material` impl)
...
- Group related parameters (Godot: `group_uniforms`, Unity: `[Header]`, Unreal: Category, Bevy: fields in the
  `AsBindGroup` material struct)
