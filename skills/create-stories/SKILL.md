...
**Primary role:** Read `../../agents/lead-programmer.md` before following this workflow.

# Create Stories

A story is a single implementable behaviour — small enough to complete in one focused session, self-contained, and fully
traceable to a GDD requirement and an ADR decision. Stories are what developers pick up. Epics are what architects
define.

**Run this skill per epic**, not per layer. Run it for Foundation epics first, then Core, and so on — matching the
dependency order.
...
Extract `--review [full|lean|solo]` if present and store as the review mode override for this run. If not provided, read
...
**Story sizing rule:** one story = one focused session (~2-4 hours). If a group of criteria would take longer, split
into two stories.
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `gamedev:qa-lead`
through authorized delegation using gate **QL-STORY-READY** (`../../docs/director-gates.md`).
...
For each story `.md` just written, create the tracking task with Backlog `task_create`:
...
Then edit the story `.md` header's `Tracked in: TASK-N` placeholder to the real task ID returned by `task_create`. The
task (status) and the `.md` (spec + evidence trail) now cross-reference each other.

### Also update `production/epics/[epic-slug]/EPIC.md`

Replace the "Stories: Not yet created" line with a populated table. **No Status column** — status lives in the Backlog
task:
...
Use a user-input tool or chat to close with context-aware next steps:
