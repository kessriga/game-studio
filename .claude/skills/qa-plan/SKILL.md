...
Critical paths to verify before any QA hand-off for this milestone:

1. Game launches to main menu without crash
2. New game / new session can be started
3. [Primary mechanic introduced or changed this milestone]
4. [Any system with a regression risk from this milestone's changes]
...
`production/session-logs/playtest-[milestone]-[story-slug].md` and reviewed by
...
- [ ] Smoke check passes (run `/smoke-check` before QA hand-off)
...
- Once all milestone stories are implemented, run `/smoke-check` to gate QA hand-off — not yet, only after implementation is complete
...
```
<!-- QA-PLAN: [date] | System: [system/milestone identifier] | Plan written: production/qa/qa-plan-[identifier]-[date].md -->
```
