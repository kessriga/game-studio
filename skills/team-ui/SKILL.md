...
**Arguments:** [UI feature description] [--review full|lean|solo]
...
**Director gate skip rule**: Before spawning creative-director, art-director, or any other Tier 1/2 director for review
(outside of PHASE-GATE triggers), apply the resolved mode: skip if solo mode; skip if lean mode and this is not a
PHASE-GATE.
...
- **engine UI specialist** — Validates UI implementation patterns against engine-specific best practices (read from
  `docs/technical-preferences.md` Engine Specialists → UI Specialist)
...
- `gamedev:ux-designer` — User flows, wireframes, accessibility, input handling
- `gamedev:ui-programmer` — UI framework, screens, widgets, data binding
- `gamedev:art-director` — Visual style, layout polish, art bible consistency
- `role: [UI engine specialist]` — Engine-specific UI pattern validation (e.g., unity-ui-specialist, ue-umg-specialist,
  godot-specialist)
- `gamedev:accessibility-specialist` — Accessibility compliance audit
...
but this must be a conscious decision — present the specific concerns via a user-input tool or chat before asking
whether to proceed.
...
Before implementation begins, spawn the **engine UI specialist** (from `docs/technical-preferences.md` Engine
...
- Which engine UI framework should be used for this screen? (e.g., UI Toolkit vs UGUI in Unity, Control nodes vs
  CanvasLayer in Godot, UMG vs CommonUI in Unreal, bevy_ui + feathers/ui_widgets in Bevy)
...
polish). Verdict: **BLOCKED** — pipeline halted; surface the blocker and its phase before stopping.
