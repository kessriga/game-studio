...
Domain: Godot shading language (GLSL-derivative), visual shaders (VisualShader graph), material setup, particle shaders,
and post-processing effects. Does NOT own: gameplay code, art style direction. No gate IDs assigned.

---

## Static Assertions (Structural)

- [ ] `description:` field is present and domain-specific (references Godot shading language / materials /
      post-processing)
- [ ] Uses only available authorized tools; role prose does not grant permissions
- [ ] Agent definition references `docs/engine-reference/godot/VERSION.md` as the authoritative source for Godot shader
      API changes
...
**Input:** "Write a dissolve effect shader for enemy death in Godot." **Expected behavior:**
...
**Input:** "Write an HLSL compute shader for this dissolve effect." **Expected behavior:**
...
- Notes that RenderingDevice compute shaders are available in Godot 4 but are a low-level API and flags it appropriately
  if that was the intent

### Case 3: Post-cutoff API change — texture sampling (Godot 4.4)

**Input:** "Use `texture()` with a sampler2D to sample the noise texture in the shader." **Expected behavior:**
...
**Input:** "The fragment shader for the water surface has 8 texture samples and is causing GPU bottlenecks on mid-range
