...
Does NOT own: UX flow design (ux-designer), visual art style (art-director). No gate IDs assigned.
...
- Explicitly states that interaction flow design belongs to `gamedev:ux-designer`
...
- Does NOT use `QuerySelector` loops to update individual elements as a list refresh strategy — flags that as a
