...
`/gamedev:ux-design` is a guided, section-by-section UX spec authoring skill. It produces
...
The skill has no inline director gates — `/gamedev:ux-review` is the separate review step.
...
None. `/gamedev:ux-design` has no inline director gates. `/gamedev:ux-review` is the separate
...
- Interaction state enumeration (normal/hover/focus/disabled/error) is a core
  requirement of each spec; the `/gamedev:ux-review` skill checks for completeness.
